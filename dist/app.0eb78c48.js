...
function transformToObjects(numberArray) {
  return numberArray.map(function (numEl) {
    return {
      val: numEl
    };
  });
...
var _name$age = {
  ...
  userName = _name$age.name,
  age = _name$age.age;
console.log(userName, age);

/* DESTRUKTURYZACJA W PARAMETRZE FUNKCJI */
function storeOrder(_ref) {
  var id = _ref.id,
    currency = _ref.currency,
    amount = _ref.amount;
  localStorage.setItem('id', id);
  localStorage.setItem('currency', currency);
  var storageCheck = localStorage.getItem('id');
...
var hobbies = ['play', 'walk'];
/* 1. przez napisanie [] */
var newHobbies = ['Read'];

/* 2. Przez spreed operator */
var mergedHobbies = [].concat(newHobbies, hobbies);
console.log(mergedHobbies);

/* SRREED OPERATOR WOBJekcie */

var extendedUser = _objectSpread({
  isAdmin: true
}, user);
